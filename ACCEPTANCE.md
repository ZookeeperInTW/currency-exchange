...
WHEN 輸入內容改變
THEN 卡片底部單一「等值台幣」金額立即依「現金匯率（銀行買入價）」重新計算並顯示，不需按任何按鈕、不需重新整理頁面；三個輸入框各自不再顯示個別換算結果

## AC3 — 換算結果數字正確
GIVEN 美元現金匯率為 31.8950，其餘輸入框皆為空
WHEN 使用者在美元輸入框輸入 100
THEN 底部等值台幣顯示為 $3,189.50（= 100 × 31.8950）

## AC3.1 — 多欄位同時輸入時加總計算
GIVEN 美元現金匯率為 31.8950、日圓現金匯率為 0.1898
WHEN 使用者同時在美元輸入框輸入 100、日圓輸入框輸入 1000
THEN 底部等值台幣顯示為兩者加總（= 100 × 31.8950 + 1000 × 0.1898）
